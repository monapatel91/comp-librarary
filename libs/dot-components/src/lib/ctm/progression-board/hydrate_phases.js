...
export const hydratePhases = (
  ...
  phases.forEach((phase) => {
    phase.packageVersions.forEach((packageVersion) => {
      ...
      packageVersion.severity1Label = `${packageVersion.risk_severity1_violation_count} Severity One Violations`;
      packageVersion.severity1Url = packageVersion.risk_severity1_report_url;
      packageVersion.hasFailedTests = packageVersion.risk_has_failed_tests;
      packageVersion.failedTestsLabel = `${packageVersion.risk_failed_tests_count} Failed Tests`;
      packageVersion.failedTestsUrl =
        packageVersion.risk_failed_tests_report_url;
      packageVersion.hasLowCoverage = packageVersion.risk_has_low_coverage;
      packageVersion.coverageLabel = `${packageVersion.risk_coverage_percentage}% Coverage`;
      packageVersion.coverageUrl = packageVersion.risk_coverage_report_url;

      packageVersion.hasUnmanaged = packageVersion.unmanaged_change_count > 0;
      packageVersion.hasRisky = packageVersion.riskyFileCount > 0;
      packageVersion.riskyFileLabel = `${packageVersion.riskyFileCount} Risky Files.`;
      packageVersion.unManagedCommitLabel = `${packageVersion.unmanaged_change_count} Rogue Commits.`;
      packageVersion.rangeIsInvalid =
        packageVersion.rev_from > packageVersion.rev_to;
      packageVersion.revurl = `/flow/package_revision?id=${packageVersion.rev_to_id}&from_revision=${packageVersion.rev_from}`;
      packageVersion.revisionRangeLabel =
        packageVersion.rev_from + ' - ' + packageVersion.rev_to;
      const fromlbl =
        packageVersion.fullversion_from || packageVersion.rev_from;
      const tolbl = packageVersion.fullversion_to || packageVersion.rev_to;
...
export const identifySelectedPackageVersions = (
  ...
  getNewPackageVersions(phases, decoratePackageVersion(selectedPackageVersion));
